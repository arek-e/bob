...
ADR 0016 made optional domains Vertical Modules. The Core Runtime still put their rules in entrypoints.
...
The Cloudflare Adapter also needs one atomic D1 batch for delivery and target state changes.
...
A delivery target Adapter returns D1 batch statements for the Cloudflare Adapter. Delivery adds these
statements to its atomic state change. It does not use a post-commit hook.
...
Implementation in the Cloudflare Adapter.
