...
| 16  | LLM observability                       | **Partial**          | Trace context spans services. Loki has redacted agent logs. The live Grafana dashboard joins logs, pod metrics, and Tempo. | Deploy native agent traces. Export Worker telemetry. Verify SLO and alert delivery.    |
...
- Observability contracts: [`packages/observability/src/events.ts`](../packages/observability/src/events.ts)
