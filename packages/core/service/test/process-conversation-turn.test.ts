...
import { processConversationTurn, processConversationTurnEffect } from "../src/process-inbound.ts"
...
describe("conversation turn processing", () => {
  ...
  it("runs one immutable request for the newest revision and publishes one reply", async () => {
    ...
    const claimRun = vi.fn(async () => attemptId)
    ...
    const processing = Effect.runPromise(
      processConversationTurnEffect(snapshot, bindings, composition).pipe(
    ...
    expect(claimRun).toHaveBeenCalledWith(capturedRequest?.runId, 140_000)
  ...
  it.each([
    ...
    async ({
      ...
      expect(mutationActivity).toHaveBeenCalledTimes(activity.status === "active" ? 2 : 1)
