...
export function makeToolExecutor(
  ...
  function encodePrivate<Input>(ownerId: string, value: Input) {
    return Effect.gen(function* () {
      const ownerDataKey = yield* fromPromise(() => ownerDataKeys.load(ownerId))
      const encrypted = yield* fromPromise(() =>
        protection.encryptText(ownerDataKey.key, JSON.stringify(value))
      )
      return JSON.stringify(encrypted)
    })
  }

  function decodePrivate(ownerId: string, value: string) {
    return Effect.gen(function* () {
      const encrypted = Schema.decodeUnknownSync(
        Schema.Struct({ ciphertext: Schema.String, iv: Schema.String })
      )(JSON.parse(value))
      const ownerDataKey = yield* fromPromise(() => ownerDataKeys.load(ownerId))
      const plaintext = yield* fromPromise(() =>
        protection.decryptText(ownerDataKey.key, encrypted)
      )
  ...
  function runContext(runId: string) {
    return Effect.gen(function* () {
      const [row] = yield* database
        ...
        .limit(1)
      if (row === undefined) return yield* Effect.fail(new Error("Agent run not found"))
      ...
      const ownerDataKey = yield* fromPromise(() => ownerDataKeys.load(row.run.userId))
      const plaintext = yield* fromPromise(() => protection.decryptText(ownerDataKey.key, envelope))
      const request = Schema.decodeUnknownSync(AgentRunRequest)(JSON.parse(plaintext))
      if (request.sourceMessageId !== row.inbound.messageId) {
        return yield* Effect.fail(
          new Error("Agent source snapshot does not match the inbound message")
        )
      }
      return {
        request,
  ...
  function runCanClaimTool(runId: string) {
    return Effect.gen(function* () {
  ...
  function conversationTurnForRun(runId: string) {
    return Effect.gen(function* () {
      const [run] = yield* database
        .select({
  ...
  const executor: ToolExecutorService = {
    mutationActivity: (runId) =>
      Effect.gen(function* () {
        const [run] = yield* database
          ...
          .where(eq(agentRuns.id, runId))
          .limit(1)
        if (run === undefined) return { status: "none" } as const
        ...
        const active = rows.filter(
          (row) =>
            (row.status === "pending" || row.status === "executing") && row.resultJson === null
        )
        if (active.length > 0) {
        ...
        const completed = rows.filter(
          (row) => row.status === "completed" && row.resultJson !== null
        )
      ...
      }).pipe(Effect.mapError((cause) => executorError("mutationActivity", cause))),
    expireMutationRecovery: (runId) =>
      Effect.gen(function* () {
        ...
        const currentTime = now()
        const resultJson = yield* encodePrivate(run.ownerId, {
          ok: false,
    ...
    execute: (input) =>
      Effect.gen(function* () {
        ...
        return yield* withBobSpan(
          ...
          Effect.gen(function* () {
            const commandRun = yield* conversationTurnForRun(command.runId)
              ...
              command.idempotencyKey !==
                (yield* fromPromise(() =>
                  conversationMutationIdempotencyKey({
                    ...
                    toolName: command.name,
                    arguments: command.arguments,
                    excludedArgumentNames: registry.catalogue.mutationArgumentExclusions(
                      command.name
                    )
                  })
                ))
            ...
            if (stableMutationTurnId !== undefined) {
              ...
              const reserved = yield* reserveMutation(
              ...
              if (!reserved) {
                if (!(yield* runCanClaimTool(command.runId))) return denied()
                return additionalMutationConfirmationRequired()
              }
            }
            const commandHash = yield* fromPromise(() => toolCommandHash(command))
            ...
            const existingTurnId =
              existing === undefined
                ? undefined
                : ((yield* conversationTurnForRun(existing.runId))?.conversationTurnId ?? undefined)
            ...
            if (existing?.resultJson !== null && existing?.resultJson !== undefined) {
              ...
              return yield* decodePrivate(command.ownerId, existing.resultJson)
            }

            const id = randomUuid()
            const argumentsJson = yield* encodePrivate(command.ownerId, command.arguments)
            yield* database
              .insert(toolCalls)
              .values({
                id,
            ...
            const winnerTurnId =
              winner === undefined
                ? undefined
                : ((yield* conversationTurnForRun(winner.runId))?.conversationTurnId ?? undefined)
            ...
            if (winner.resultJson !== null) {
              ...
              return yield* decodePrivate(command.ownerId, winner.resultJson)
            }
            ...
            if (
              ...
              const expiredResultJson = yield* encodePrivate(command.ownerId, expiredOutcome)
              const [markedUnknown] = yield* database
                .update(toolCalls)
                .set({
                  resultJson: expiredResultJson,
                  status: "unknown",
                  claimToken: null,
                  claimExpiresAt: null
                })
              ...
              if (settled?.resultJson !== null && settled?.resultJson !== undefined) {
                return yield* decodePrivate(command.ownerId, settled.resultJson)
              }
              return {
                ok: false,
              ...
              Effect.gen(function* () {
                      ...
                      stableMutationTurnId === undefined
                        ? exists(
                            database
                              ...
                              .from(agentRuns)
                              .where(
                                and(eq(agentRuns.id, toolCalls.runId), conversationTurnAuthority())
                              )
                          )
                        : commandRunAuthority,
                      or(
                ...
                if (claimed === undefined) {
                  ...
                  const settledTurnId =
                    settled === undefined
                      ? undefined
                      : ((yield* conversationTurnForRun(settled.runId))?.conversationTurnId ??
                        undefined)
                ...
                const result = Exit.isSuccess(domainExit)
                  ...
                  : Cause.hasInterruptsOnly(domainExit.cause)
                    ? (expiredToolCallOutcome(command.name, registry.catalogue) ?? domainError())
                    : domainError()
                const resultJson = yield* encodePrivate(command.ownerId, result)
                const [settled] = yield* database
                  .update(toolCalls)
                  .set({
                    resultJson,
                    status:
                      result.evidence?.actionOutcome === "unknown"
                        ? "unknown"
                        : result.ok
                          ? "completed"
                          : "failed",
      ...
      }).pipe(Effect.mapError((cause) => executorError("execute", cause)))
  }
  return executor
}
