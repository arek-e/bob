...
export function makeMemoryStore(
  ...
  function releaseReviewGuard(claim: ReviewClaim): DatabaseQuery {
    return database
  ...
  return {
    ...
    async confirm(ownerId, candidateId, authority, idempotencyKey) {
      ...
      let [fact] = await Effect.runPromise(
        database
          .select()
          .from(facts)
          .where(
