...
export function makeTrainingStore(
  ...
  return {
      ...
      try {
        await Effect.runPromise(
          allInTransaction(database, [
            database
              .insert(gyms)
              .values({ id, userId: ownerId, name, createdAt: at, updatedAt: at }),
    ...
    async addEquipment(ownerId, gymId, name, identifier, idempotencyKey) {
      ...
      const previous = await completedEffect(database, effect)
      if (previous !== undefined) return previous
      const [gym] = await Effect.runPromise(
        database
          .select({ id: gyms.id })
          .from(gyms)
    ...
    async saveRoutine(input, idempotencyKey) {
      ...
      if (input.approvalEvidence.sourceType === "owner_message") {
        const [approvalMessage] = await Effect.runPromise(
          database
        ...
        const [approvalProposal] = await Effect.runPromise(
          database
