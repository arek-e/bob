...
import type { EgressBindings } from "@bob/sendblue-channel/egress/bindings"
import type { IngressBindings } from "@bob/sendblue-channel/ingress/bindings"
...
import egressWorker from "@bob/sendblue-channel/egress"
import { processOutboundJob } from "@bob/sendblue-channel/egress/queue"
import { handleIngressHttp } from "@bob/sendblue-channel/ingress/http"
...
function makeHttpFetcher(baseUrl: string): Fetcher {
  // SAFETY: Channel Modules only use Fetcher.fetch. This Adapter cannot open raw sockets.
  return {
    fetch(input, init) {
...
function makeQueueBinding<Job>(publisher: JobPublisher<Job>): Queue<Job> {
  // SAFETY: Channel compositions only call Queue.send, which maps to durable publication.
  return { send: (job: Job) => publisher.publish(job) } as never
}
