  ...
  return {
    ...
    async deleteEntry(ownerId, entryId, idempotencyKey) {
      ...
      const sourcedRevisions = await database
        ...
        .where(
          and(
            eq(factEvidence.sourceType, "journal"),
      ...
      for (const sourced of sourcedRevisions) {
        ...
        const hasOtherEvidence = evidence.some(
          (item) =>
            ...
            (item.sourceType !== "journal" || item.sourceId !== entryId)
        )
        ...
        database
          ...
          .where(
            and(
              ...
              eq(memoryCandidates.sourceType, "journal"),
              eq(memoryCandidates.sourceId, entryId)
            )
          ),
        database
          .delete(factEvidence)
          .where(and(eq(factEvidence.sourceType, "journal"), eq(factEvidence.sourceId, entryId)))
      ]
